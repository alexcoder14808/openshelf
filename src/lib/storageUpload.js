 ...
 * at once, instead of one-at-a-time — this is what makes uploading a
 * 100-chapter audiobook take under a minute instead of several. Reports
 * progress after each individual item finishes (not just per batch), and
 * checks `isCancelled()` between items so a mid-upload Cancel button can
 * actually stop new uploads from starting (in-flight ones still finish;
 * true network-level abort isn't attempted here, this is cooperative
 * cancellation, which is enough to stop the wait growing any longer).
 */
export async function runWithConcurrency(items, worker, { concurrency = 4, onProgress, isCancelled } = {}) {
