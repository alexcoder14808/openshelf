...
/**
 * Best-effort server sync so "time listened" can be shown on the Loans
 * page (and progress survives switching devices). Fire-and-forget — a
 * failed sync shouldn't interrupt playback, localStorage is still the
 * source of truth for resuming within this browser.
 */
function syncPositionToServer(userId, trackId, seconds) {
  if (!userId || !trackId) return;
  supabase
    .from('playback_progress')
    .upsert(
      { user_id: userId, track_id: trackId, position_seconds: Math.floor(seconds) },
      { onConflict: 'user_id,track_id' }
    )
    .then(({ error }) => {
      if (error) console.error('Failed to sync progress to server:', error);
    });
}
...
export function AudioPlayerProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const onPause = () => {
      ...
      if (track) syncPositionToServer(userIdRef.current, track.id, audio.currentTime);
    };
  ...
  useEffect(() => {
    progressSaveIntervalRef.current = window.setInterval(() => {
      ...
      if (audio && track && !audio.paused) {
        ...
        syncPositionToServer(userIdRef.current, track.id, audio.currentTime);
      }
    }, 5000);
  ...
  const loadTrack = useCallback(async (track, { autoplay = false, resumeSeconds = null } = {}) => {
    ...
    if (targetSeconds > 0) {
      // Resuming mid-chapter needs real metadata before we can safely seek
      // to a specific point, so we wait for it here.
      audio.addEventListener(
        'loadedmetadata',
        function onceLoaded() {
          if (loadTokenRef.current !== myToken) return; // a newer load superseded us
          const dur = audio.duration;
          audio.currentTime = Number.isFinite(dur) ? Math.min(targetSeconds, Math.max(dur - 0.5, 0)) : targetSeconds;
      ...
      // Starting at 0:00 — no seek needed, so don't make autoplay wait on a
      // network round-trip first. Calling play() immediately, in the same
      // tick as the user's click, is what keeps autoplay reliable: waiting
      // for 'loadedmetadata' before calling play() can lose the browser's
      // "this was triggered by a user gesture" credit on a slow connection
      // (large chapter files in particular), which is what caused
      // chapter-switches to sometimes silently fail to play.
      setCurrentTime(0);
      audio.addEventListener('loadedmetadata', function markLoaded() {
        if (loadTokenRef.current === myToken) setIsLoaded(true);
      }, { once: true });
      if (autoplay) {
        audio.play().catch((err) => console.warn('Autoplay was blocked:', err));
      }
  ...
  const goToTrackIndex = useCallback(
    async (index, { autoplay = true } = {}) => {
      ...
      if (audio && outgoing) {
        savePosition(outgoing.id, audio.currentTime);
        syncPositionToServer(userIdRef.current, outgoing.id, audio.currentTime);
      }

      setCurrentTrackIndex(index);
      // Manually navigating (Next/Previous/selecting from the Chapters
      // panel) ALWAYS restarts the target chapter at 0:00 — it must never
      // inherit that chapter's own last-saved resume position. Only the
      // very first load of a book (loadBook/loadBookById) or explicitly
      // jumping to a bookmark should resume mid-chapter.
      await loadTrack(tracks[index], { autoplay, resumeSeconds: 0 });
    },
  ...
  const nextChapter = useCallback(() => {
    ...
    if (idx >= tracks.length - 1) return; // already last — no-op, no crash
    goToTrackIndex(idx + 1, { autoplay: true });
  }, [tracks.length, goToTrackIndex]);
