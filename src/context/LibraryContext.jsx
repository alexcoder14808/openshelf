...
const LOCKED_LIBRARIES_KEY = 'openshelf-locked-library-ids';

function loadLockedLibraries() {
  try {
    const raw = localStorage.getItem(LOCKED_LIBRARIES_KEY);
    return raw ? new Set(JSON.parse(raw)) : new Set();
  } catch {
    return new Set();
  }
}
function saveLockedLibraries(set) {
  try {
    localStorage.setItem(LOCKED_LIBRARIES_KEY, JSON.stringify([...set]));
  } catch (err) {
    console.error('Failed to persist locked libraries:', err);
  }
}
...
export function LibraryProvider({ children }) {
  ...
  // Libraries the user explicitly "logged out of" — re-entering one of
  // these requires re-verifying card number + PIN, even though the
  // OpenShelf account itself is still signed in.
  const [lockedLibraryIds, setLockedLibraryIds] = useState(loadLockedLibraries);
  ...
  // Keep the active library valid — default to the first UNLOCKED active
  // membership (a locked one must be re-verified, not silently auto-picked).
  useEffect(() => {
    if (!memberships.length) return;
    const stillValid =
      memberships.some((m) => m.library.id === activeLibraryId && m.status === 'active') &&
      !lockedLibraryIds.has(activeLibraryId);
    if (!stillValid) {
      const first = memberships.find((m) => m.status === 'active' && !lockedLibraryIds.has(m.library.id));
      if (first) setActiveLibrary(first.library.id);
      else setActiveLibrary(null);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [memberships, lockedLibraryIds]);
  ...
  // "Log out of [Library]" — the account stays signed into OpenShelf, but
  // this specific library now requires re-entering card number + PIN.
  const lockLibrary = useCallback((libraryId) => {
    setLockedLibraryIds((prev) => {
      const next = new Set(prev);
      next.add(libraryId);
      saveLockedLibraries(next);
      return next;
    });
  ...
  const isLibraryLocked = useCallback((libraryId) => lockedLibraryIds.has(libraryId), [lockedLibraryIds]);

  // Re-entering a locked library: verify card number + PIN server-side
  // (verify_own_card_pin is scoped to the caller's own card, so this can't
  // be used to test anyone else's), then unlock and switch to it.
  const verifyCardPinToUnlock = useCallback(async (libraryId, cardNumber, pin) => {
    const { data, error } = await supabase.rpc('verify_own_card_pin', {
      p_library_id: libraryId,
      p_card_number: cardNumber,
      p_pin: pin,
    });
    if (error) throw error;
    if (!data) throw new Error('Invalid card number or PIN.');
