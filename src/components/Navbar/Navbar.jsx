...
import { Library, ChevronDown, ShieldCheck, LogOut, User, Clock, PlusCircle, DoorOpen, Lock } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { useLibrary } from '../../context/LibraryContext';
import LibraryReauthModal from './LibraryReauthModal';
import './Navbar.css';

export default function Navbar() {
  const { isAuthenticated, profile, signOut } = useAuth();
  const { memberships, adminLibraries, activeLibrary, setActiveLibrary, lockLibrary, isLibraryLocked } = useLibrary();
  const [switcherOpen, setSwitcherOpen] = useState(false);
  const [accountOpen, setAccountOpen] = useState(false);
  const [reauthLibrary, setReauthLibrary] = useState(null);
  ...
  // Logs out of just this library — clears it as active but, critically,
  // also marks it "locked": getting back in requires the card number + PIN
  // again, even though the OpenShelf account itself stays signed in.
  const handleLogOutOfLibrary = () => {
    setAccountOpen(false);
    if (activeLibrary) lockLibrary(activeLibrary.id);
    navigate('/');
  };

  const handleSwitcherItemClick = (membership) => {
    if (isLibraryLocked(membership.library.id)) {
      setSwitcherOpen(false);
      setReauthLibrary(membership.library);
      return;
    }
    setActiveLibrary(membership.library.id);
    setSwitcherOpen(false);
  };
  ...
  return (
    <header className="navbar">
                  ...
                  {memberships.map((m) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {locked && <Lock size={13} style={{ marginLeft: 'auto', color: '#9aa0ab' }} />}
                        {!locked && m.status !== 'active' && <span className="library-switcher-pending">Pending</span>}
                      </button>
                    );
                  })}
      ...
      {reauthLibrary && (
        <LibraryReauthModal
          library={reauthLibrary}
          onClose={() => setReauthLibrary(null)}
          onUnlocked={() => setReauthLibrary(null)}
        />
      )}
    </header>
  );
