...
export default function CatalogPanel({ libraryId }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18, flexWrap: 'wrap', gap: 12 }}>
        <p style={{ margin: 0, color: '#6b7280', fontSize: '0.9rem' }}>{books.length} titles in this library's collection.</p>
        <Link
          to={`/admin/${libraryId}/add-book`}
          className="form-submit"
          style={{ width: 'auto', padding: '9px 16px', display: 'inline-flex', alignItems: 'center', gap: 6, textDecoration: 'none' }}
        >
          ...
          {books.map((book) => (
            <li className="card-list-item" key={book.id}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                {book.cover_url ? (
                  <img src={book.cover_url} alt="" style={{ width: 40, height: 56, objectFit: 'cover', borderRadius: 6, flexShrink: 0 }} />
                ) : (
                  <div style={{ width: 40, height: 56, borderRadius: 6, flexShrink: 0, background: 'linear-gradient(135deg,#5b6df5,#8c5bf5)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '0.9rem' }}>
                    {book.title?.[0]}
                  </div>
                )}
                <span><strong>{book.title}</strong><br /><span style={{ color: '#6b7280', fontSize: '0.85rem' }}>{book.author}</span></span>
              </span>
