...
export default function AudioPlayer() {
  const {
    ...
    bookmarks, sleepTimerEndsAt, loadError, isLoaded,
  ...
  // Audible-style 3-part time row: elapsed (left) / time left in the whole
  // book (center) / negative countdown to the end of THIS chapter (right).
  // Book-remaining only renders once every chapter has a known duration —
  // older uploads made before duration capture existed may not have one,
  // in which case we simply omit the center label rather than show a
  // misleading partial total.
  const chapterRemaining = Number.isFinite(duration) ? Math.max(duration - currentTime, 0) : 0;
  const bookRemaining = useMemo(() => {
    if (!Number.isFinite(duration)) return null;
    let total = chapterRemaining;
    for (let i = currentTrackIndex + 1; i < tracks.length; i++) {
      const d = tracks[i]?.duration_seconds;
      if (!Number.isFinite(d)) return null; // unknown duration somewhere — don't show a partial total
      total += d;
    }
    return total;
  }, [tracks, currentTrackIndex, duration, chapterRemaining]);
    ...
    <div className={`audio-player ${fullscreen ? 'audio-player--fullscreen' : ''}`}>
      <div className="audio-player-card">
        ...
        <div className="audio-player-time-row">
          <span className="audio-player-time audio-player-time--elapsed">{formatTime(currentTime)}</span>
          {bookRemaining !== null && (
            <span className="audio-player-time audio-player-time--center">{formatTime(bookRemaining)} left in book</span>
          )}
          <span className="audio-player-time audio-player-time--countdown">-{formatTime(chapterRemaining)}</span>
        </div>

        <div className="audio-player-transport">
          <button className="transport-btn" onClick={previousChapter} disabled={tracks.length <= 1 || !isLoaded} aria-label="Previous chapter">
            <SkipBack size={22} />
          </button>
          ...
          <button className="transport-btn" onClick={nextChapter} disabled={isLastChapter || !isLoaded} aria-label="Next chapter">
            <SkipForward size={22} />
          </button>
        </div>
      ...
      <SlidePanel open={openPanel === 'volume'} title="Volume" onClose={closePanel}>
        <div className="panel-volume-row">
          ...
          <input
            ...
            style={{
              background: `linear-gradient(to right, #5b6df5 ${muted ? 0 : Math.round(volume * 100)}%, #eceef1 ${muted ? 0 : Math.round(volume * 100)}%)`,
            }}
          />
