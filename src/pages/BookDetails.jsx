...
import { Headphones, BookOpen, Clock, Users, Lock } from 'lucide-react';
...
export default function BookDetails() {
  ...
  const { book: loadedBook, stopAndUnload } = useAudioPlayer();
  ...
  if (isLibraryLocked(book.library_id)) {
    return (
      <div className="page">
        <div className="empty-state">
          <Lock size={28} style={{ marginBottom: 10, color: '#9aa0ab' }} />
          <h2>You're logged out of this library</h2>
          <p>Log back in with its card number and PIN to view this title.</p>
          <Link to="/">Go to OpenShelf</Link>
        </div>
      </div>
    );
  }
      ...
      <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap' }}>
        {book.cover_url ? (
          <img
            src={book.cover_url}
            alt=""
            style={{ width: 220, height: 'auto', borderRadius: 16, boxShadow: '0 8px 24px rgba(15,23,42,0.15)', display: 'block', flexShrink: 0 }}
          />
        ) : (
        ...
        <div style={{ flex: 1, minWidth: 260 }}>
          ...
          {book.description && <p style={{ color: '#40444c', lineHeight: 1.7, maxWidth: 560, whiteSpace: 'pre-line' }}>{book.description}</p>}
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                ...
                <button
                  ...
                  onClick={() => runAction(async () => {
                    ...
                    // If this exact book was loaded in the global player
                    // (e.g. paused in the background while browsing), returning
                    // it must actually stop access — not just remove the row.
                    if (loadedBook?.id === book.id) stopAndUnload();
                  })}
                >
                  Return early
                </button>
