...
export default function Home() {
  ...
  return (
    <div className="page">
      ...
      {/* Signed into OpenShelf, holds at least one card, but no library is
          currently active — e.g. they logged out of a library (which locks
          it, requiring card+PIN again) without picking another. This is
          deliberately its own "OpenShelf only" screen, not a blank page —
          the library and OpenShelf identities stay separate, same as
          Libby/OverDrive. */}
      {isAuthenticated && memberships.length > 0 && !activeLibrary && (
        <div className="empty-state">
          <h2>You're signed into OpenShelf</h2>
          <p>Choose a library from the switcher above to browse and borrow.</p>
          <Link to="/library-card">View your library cards</Link>
        </div>
      )}
