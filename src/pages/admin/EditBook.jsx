...
import { Plus, Trash2, Loader2, CheckCircle2, Lock } from 'lucide-react';
import { supabase } from '../../lib/supabaseClient';
import { useLibrary } from '../../context/LibraryContext';
import { uploadCover, uploadAudioTrack, uploadEbookFile, guessEbookFileType, prepareChapterFolder, getAudioDuration, runWithConcurrency } from '../../lib/storageUpload';
...
export default function EditBook() {
  ...
  const { isLibraryLocked } = useLibrary();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const tracksNeedingUpload = tracksWithChanges.filter((t) => t.newFile);

      // Upload every changed/new chapter file in parallel (up to 8 at once)
      // instead of one at a time — same fix as Add Book, for the same
      // reason: bulk-appending a folder of new chapters here hits the exact
      // same slowness otherwise.
      let uploadResultsByTrack = new Map();
      if (tracksNeedingUpload.length > 0) {
        setUploadProgress({ done: 0, total: tracksNeedingUpload.length });
        const results = await runWithConcurrency(
          ...
          },
          {
            ...
            onProgress: (done, total) => {
              ...
              setProgressLabel(`Uploading chapter files… (${done} of ${total})`);
            },
          }
        );
        const failed = results.filter((r) => !r.ok);
        if (failed.length > 0) {
          console.error('Some chapter uploads failed:', failed);
          throw new Error(`${failed.length} of ${tracksNeedingUpload.length} chapter files failed to upload. Please try again.`);
        }
        uploadResultsByTrack = new Map(tracksNeedingUpload.map((t, i) => [t, results[i].value]));
        setUploadProgress(null);
      }

      setProgressLabel('Saving chapters…');
      await Promise.all(
        tracksWithChanges.map((t) => {
          const uploaded = uploadResultsByTrack.get(t);
          const audioUrl = uploaded?.audioUrl ?? t.audio_url;
          const durationSeconds = uploaded?.durationSeconds ?? (t.duration_seconds ?? null);
          if (!audioUrl) return Promise.resolve(); // new row with no file chosen yet — skip
          ...
          if (t.id) {
            return supabase
              ...
              .eq('id', t.id)
              .then(({ error: updErr }) => { if (updErr) throw updErr; });
          }
          return supabase
            .from('audiobook_tracks')
            .insert({
            ...
            .then(({ error: insErr }) => { if (insErr) throw insErr; });
        })
      );
    ...
    return (
      <div className="page">
        <div className="empty-state">
          ...
          <h2>You're logged out of this library</h2>
          <Link to={`/admin/${libraryId}`}>Log back in to continue</Link>
        </div>
      </div>
    );
  }
        ...
        {uploadProgress && (
          <div style={{ marginBottom: 16 }}>
            ...
            <p style={{ fontSize: '0.78rem', color: '#6b7280', margin: '6px 0 0' }}>
              {uploadProgress.done} of {uploadProgress.total} chapter files uploaded
            </p>
          </div>
        )}
