  ...
  return (
    <div className="page">
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 24, flexWrap: 'wrap' }}>
        <select value={genre} onChange={(e) => setGenre(e.target.value)} className="pill-select">
          {genres.map((g) => (
            <option key={g} value={g}>{g}</option>
          ))}
        </select>
        <select value={format} onChange={(e) => setFormat(e.target.value)} className="pill-select">
