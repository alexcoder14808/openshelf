...
export default function Dashboard() {
  ...
  const { book, hasActiveBook, currentTrack, currentTrackIndex, currentTime, stopAndUnload } = useAudioPlayer();
              ...
              {activeCheckouts.map((c) => (
                <li className="card-list-item" key={c.id}>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        await returnBook(c.id);
                        if (book?.id === c.book?.id) stopAndUnload();
                      } catch (err) {
                        alert(err.message);
                      }
                    }}
