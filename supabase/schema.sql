...
-- NOTE: Supabase installs extensions like pgcrypto into an "extensions"
-- schema, not "public". Any SECURITY DEFINER function that calls
-- pgp_sym_encrypt/pgp_sym_decrypt must include "extensions" in its
-- search_path (see request_library_card, reveal_my_card_pin,
-- card_login_lookup below) or it fails with "function ... does not exist"
...
create table if not exists profiles (
  ...
  terms_accepted_at timestamptz, -- evidence of consent to Terms of Service + Privacy Policy at signup
...
-- Deleting is owner-only (not any admin) — every dependent table already
-- cascades from libraries (library_admins, library_members, books, and
-- from books further down to audiobook_tracks/ebook_files/checkouts/
-- holds/bookmarks), so this one policy is enough to make deletion work.
...
-- verify_own_card_pin: re-verifies the CALLER's OWN card+PIN for a library
-- they're already a member of — used for "log back into this library"
-- after explicitly logging out of just that library while staying signed
-- into OpenShelf overall. Scoped to the caller's own row, so safe to grant
-- to any authenticated user directly (can't test someone else's card).
create or replace function public.verify_own_card_pin(p_library_id uuid, p_card_number text, p_pin text)
returns boolean
language plpgsql
security definer
set search_path = public, extensions
...
begin
  ...
  select * into v_row
  from library_members
  where library_id = p_library_id and user_id = v_user and card_number = p_card_number and status = 'active';

  if not found or v_row.pin_encrypted is null then
    return false;
  end if;

  return pgp_sym_decrypt(v_row.pin_encrypted, 'openshelf-card-pin-v1') = p_pin;
end;
$$;

revoke all on function public.verify_own_card_pin(uuid, text, text) from anon, public;
grant execute on function public.verify_own_card_pin(uuid, text, text) to authenticated;
